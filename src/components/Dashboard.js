...
const CardUsage = ({ cardData, transactions }) => {
  // Safety check - if cardData is undefined, use empty object
  if (!cardData) {
    return (
      <div className="space-y-6">
        <div className="bg-white/10 backdrop-blur-sm rounded-xl p-6 text-center">
          <p className="text-white">Loading card data...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="card-gradient p-6 rounded-xl text-white hover-scale">
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="text-right">
            <p className="text-2xl font-bold">${(cardData.balance || 0).toLocaleString()}</p>
            <p className="text-white/70 text-sm">{cardData.currency || 'USD'} Balance</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4 mt-6">
          <div className="bg-white/10 rounded-lg p-3">
            ...
            <p className="text-lg font-semibold">${(cardData.monthlySpending || 0).toLocaleString()}</p>
          </div>
          <div className="bg-white/10 rounded-lg p-3">
            <div className="flex items-center gap-2 mb-1">
            ...
            <p className="text-lg font-semibold">${(cardData.availableCredit || 0).toLocaleString()}</p>
          </div>
        </div>
      </div>
